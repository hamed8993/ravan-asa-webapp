...
episodeSchema.methods.download = function(check, user) {
    ...
    if ( this.type == 'vip' ) status = user.isVip()
    else if(this.type == 'cash') status = user.checkLearning(this.course);

    let timestamps = new Date().getTime() + 12*1000*3600;//تا 12 ساعت معتبر باشه
    let text = `Episode-Download-SecretKey-${this.id}${timestamps}`;
